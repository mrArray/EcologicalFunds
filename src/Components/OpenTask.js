...
export default class OpenTask extends Component {
  constructor(props) {
    super(props);
    this.state = {
      project: "",
  ...
  componentDidMount() {
    ...
    axios.get("https://ecological.chinikiguard.com/projects/api/tasks/list/?all_record=1&for_user=true&status=open",
      {
        headers:
        {
      ...
      .then(res => {
        if (res.data) {
          localStorage.setItem("AllTasksDataStatusOpen", JSON.stringify(res.data));
        }
        // (res);
        // (res.data);
