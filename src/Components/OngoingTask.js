...
export default class OngoingTask extends Component {
  constructor(props) {
    super(props);
    this.state = {
      project: "",
  ...
  componentDidMount() {
    ...
    axios.get("https://ecological.chinikiguard.com/projects/api/tasks/list/?all_record=1&for_user=true&status=inprogress",
      {
        headers:
        {
      ...
      .then(res => {
        if (res.data) {
          localStorage.setItem("AllTasksDataStatusInprogress", JSON.stringify(res.data));
        }
        // (res);
        // (res.data);
